<script lang="ts">
    ...
    onMount(async (): Promise<void>=>{
      ...
      if(!owner?.fetchNotes) return 
      await owner.fetchNotes({relays:[url]})
...
<div class="grid grid-cols-5 grid-flow-col mt-10 p-2h-80  py-5 px-5 rounded-lg bg-gradient-to-b from-gray-100 to-white">
    <div class="items-center col-span-2 ">
      {#if browser && profile}
        <h3 class="mb-2 text-gray-600 scroll-m-20 text-xl font-extrabold tracking-tight mb-2">
          relay operator
        </h3>
        <div class="profile-card  p-5 text-white max-w-xs block bg-gradient-to-r from-purple-400 via-pink-500 to-red-500 rounded-lg ">
          <div class="flex items-center space-x-4">
            ...
            {#if picture}
              <Avatar.Root class="inline-block">
                <Avatar.Image src="{picture}" alt="@{profile?.name || 'anonymous'}" />
                <Avatar.Fallback>@{profile?.name || 'anonymous'}</Avatar.Fallback>
              </Avatar.Root>
            {/if}
          ...
          <div class="my-3">{ profile?.about }</div>
